...
const { sleep, warn, request_server_region_info, check_query_text, check_language, get_headers } = require('./common');
...
const make_temp_language_map = (from_language, to_language) => {
	...
	let lang_list = [from_language, to_language];
	let lang_pair = {};
...
const google_api = async (query_text, from_language = 'auto', to_language = 'en', kwargs = {}) => {
	...
	:return: str or list
	*/
	const { reset_host_url, if_use_cn_host, is_detail_result, timeout, proxies, sleep_seconds = parseInt(Math.random() * 10), if_ignore_limit_of_length } = kwargs;
	...
	let json_data = JSON.parse(r_arr[3]);
